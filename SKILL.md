...
## 📏 输出/询问规范（LLM 必须遵守，最高优先级）

**每次与用户交互，无论询问还是输出，一律简洁：**

1. **询问**：只问必要的问题，一次问完；选项用最短形式（如 `1. 音频 2. 视频+音频`），不加多余解释、不加客套话。
2. **输出**：
   - 下载结果用紧凑列表/单行总结（文件类型、大小、链接），**禁止**粘贴完整日志、脚本输出或重复的技术细节。
   - 出错时只报一句原因 + 一句解决方案，不展开排查过程。
3. **全程**：不用 emoji 堆砌（每屏最多 1-2 个）、不重复用户已知信息、不主动推荐额外操作（除非用户要求）。
