...
### 规则1：先问再下载（流程）
收到链接/BV号/短链：
1. **先提供在线预览页**（含封面+标题+播放器+分P列表，**必须用 `bilibili-preview.sh` 生成**，给 minis:// 链接；同时附 B站直链）：
```bash
bash <skill_dir>/scripts/bilibili-preview.sh "BVxxx?p=N"
# 输出: minis://workspace/bilibili-preview/BVxxx_pN.html (预览页)
#       https://www.bilibili.com/blackboard/webplayer/mbplayer.html?bvid=BVxxx&p=N (直链)
```
```
🎬 [预览: 封面+播放](minis://workspace/bilibili-preview/BVxxx_pN.html)
